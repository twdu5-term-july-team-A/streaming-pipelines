...
nohup java -jar /tmp/tw-citibike-apis-producer0.1.0.jar --spring.profiles.active=\${station_marseille} --producer.topic=station_data_marseille --kafka.brokers=${KAFKA_HOST}:9092 1>/tmp/\${station_marseille}.log 2>/tmp/\${station_marseille}.error.log &

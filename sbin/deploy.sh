...
KAFKA_HOST1=kafka${HOST_SUFFIX}1.${TRAINING_COHORT}.training
KAFKA_HOST2=kafka${HOST_SUFFIX}2.${TRAINING_COHORT}.training
KAFKA_HOST3=kafka${HOST_SUFFIX}3.${TRAINING_COHORT}.training
...
scp ./zookeeper/seed.sh ${KAFKA_HOST1}:/tmp/zookeeper-seed.sh
ssh ${KAFKA_HOST1} <<EOF
set -e
export hdfs_server="${EMR_MASTER_HOST}:8020"
export kafka_server="${KAFKA_HOST1}:9092"
...
nohup java -jar /tmp/tw-citibike-apis-producer0.1.0.jar --spring.profiles.active=\${station_information} --kafka.brokers=${KAFKA_HOST1}:9092,${KAFKA_HOST2}:9092,${KAFKA_HOST3}:9092 1>/tmp/\${station_information}.log 2>/tmp/\${station_information}.error.log &
nohup java -jar /tmp/tw-citibike-apis-producer0.1.0.jar --spring.profiles.active=\${station_san_francisco} --producer.topic=station_data_sf --kafka.brokers=${KAFKA_HOST1}:9092,${KAFKA_HOST2}:9092,${KAFKA_HOST3}:9092 1>/tmp/\${station_san_francisco}.log 2>/tmp/\${station_san_francisco}.error.log &
nohup java -jar /tmp/tw-citibike-apis-producer0.1.0.jar --spring.profiles.active=\${station_status} --kafka.brokers=${KAFKA_HOST1}:9092,${KAFKA_HOST2}:9092,${KAFKA_HOST3}:9092 1>/tmp/\${station_status}.log 2>/tmp/\${station_status}.error.log &
nohup java -jar /tmp/tw-citibike-apis-producer0.1.0.jar --spring.profiles.active=\${station_marseille} --producer.topic=station_data_marseille --kafka.brokers=${KAFKA_HOST1}:9092,${KAFKA_HOST2}:9092,${KAFKA_HOST3}:9092 1>/tmp/\${station_marseille}.log 2>/tmp/\${station_marseille}.error.log &
...
nohup spark-submit --master yarn --deploy-mode cluster --class com.tw.apps.StationLocationApp --name StationStatusSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 500M --executor-memory 1g --executor-cores 1 /tmp/tw-raw-data-saver_2.11-0.0.1.jar ${KAFKA_HOST1}:2181,${KAFKA_HOST2}:2181,${KAFKA_HOST3}:2181 "/tw/stationStatus" 1>/tmp/raw-station-status-data-saver.log 2>/tmp/raw-station-status-data-saver.error.log &

nohup spark-submit --master yarn --deploy-mode cluster --class com.tw.apps.StationLocationApp --name StationInformationSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 500M --executor-memory 1g --executor-cores 1 /tmp/tw-raw-data-saver_2.11-0.0.1.jar ${KAFKA_HOST1}:2181,${KAFKA_HOST2}:2181,${KAFKA_HOST3}:2181 "/tw/stationInformation" 1>/tmp/raw-station-information-data-saver.log 2>/tmp/raw-station-information-data-saver.error.log &

nohup spark-submit --master yarn --deploy-mode cluster --class com.tw.apps.StationLocationApp --name StationDataSFSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 500M --executor-memory 1g --executor-cores 1 /tmp/tw-raw-data-saver_2.11-0.0.1.jar ${KAFKA_HOST1}:2181,${KAFKA_HOST2}:2181,${KAFKA_HOST3}:2181 "/tw/stationDataSF" 1>/tmp/raw-station-data-sf-saver.log 2>/tmp/raw-station-data-sf-saver.error.log &
...
nohup spark-submit --master yarn --deploy-mode cluster --class com.tw.apps.StationApp --name StationApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0  --num-executors 2 --driver-memory 500M --executor-memory 2g --executor-cores 2 /tmp/tw-station-consumer_2.11-0.0.1.jar ${KAFKA_HOST1}:2181,${KAFKA_HOST2}:2181,${KAFKA_HOST3}:2181 1>/tmp/station-consumer.log 2>/tmp/station-consumer.error.log &

nohup spark-submit --master yarn --deploy-mode cluster --class com.tw.apps.StationApp --name StationTransformerNYC --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0  --num-executors 2 --driver-memory 500M --executor-memory 2g --executor-cores 2 /tmp/tw-station-transformer-nyc_2.11-0.0.1.jar ${KAFKA_HOST1}:2181,${KAFKA_HOST2}:2181,${KAFKA_HOST3}:2181 1>/tmp/station-transformer-nyc.log 2>/tmp/station-transformer-nyc.error.log &
